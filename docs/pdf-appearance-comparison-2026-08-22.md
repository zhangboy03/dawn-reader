...
Dawn 先同时实现两种 PDF 外观策略，让用户在同一份 PDF、同一页、同一缩放和同一位置直接比较：

- **环境**：只改变工具栏、侧栏和页面周围的颜色，PDF 页面保持原样。
- **页面**：环境同步换色，并对每个完整 PDF 页面应用显示滤镜。
- **原色**：两种处理方式下都不使用页面滤镜，作为精确颜色基准。
...
用户选定处理方式后，应删除实验性的“环境 / 页面”一行，并把正式 PDF 菜单恢复为只含三种色调的极简版本。
