...
export function loadProfile(): ReaderProfile | null {
  try { return JSON.parse(readerLocalStorage().getItem(PROFILE_KEY) ?? "null"); } catch { return null; }
}

export function saveProfile(profile: ReaderProfile) {
  readerLocalStorage().setItem(PROFILE_KEY, JSON.stringify(profile));
}
...
export function addReadingNote(kind: "known" | "difficult", text: string) {
  ...
  readerLocalStorage().setItem(NOTES_KEY, JSON.stringify(notes.slice(0, 200)));
}

export function loadReadingNotes(): Array<{ kind: "known" | "difficult"; text: string; at: string }> {
  try { return JSON.parse(readerLocalStorage().getItem(NOTES_KEY) ?? "[]"); } catch { return []; }
}
